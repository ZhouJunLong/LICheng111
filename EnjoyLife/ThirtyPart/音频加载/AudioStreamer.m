...
static AudioStreamer *single = nil;
//
//单例
//
+ (AudioStreamer *)shareManager
{
    ...
    dispatch_once(&once, ^{
        if (!single) {
            single = [[AudioStreamer alloc]init];
        }
    });
...
- (BOOL)openReadStream
{
	@synchronized(self)
	{
		...
		{
			CFRelease(stream);
			[self presentAlertWithTitle:NSLocalizedStringFromTable(@"错误", @"Errors", nil)
								message:NSLocalizedStringFromTable(@"当前歌曲网络有错误", @"Errors", nil)];
			return NO;
		}
